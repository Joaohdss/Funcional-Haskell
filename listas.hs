...
compact [] = []
compact [x] = [x]
compact (x:xs) |(x `elem` xs) = [x] ++ [x] ++ compact (removeList x xs)
...
-}
conteExist e [] = 0
conteExist e (x:xs) | (e == x) = 1 + conteExist e xs
                    | otherwise = conteExist e xs

removeTodos e [] = []
removeTodos e (x:xs)
 |e == x = removeTodos e xs
 |otherwise = [x] ++ (removeTodos e xs)

encode [] = []
encode (x:xs) = (x, conteExist x (x:xs)) : encode (removeTodos x xs) 
